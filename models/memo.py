...
class MEMO(BaseLearner):
    ...
    # 在每个任务完成后执行的操作，包括更新已知类别数、设置模型的状态（训练或评估）、记录 exemplar（记忆数据）大小。
    def after_task(self):
    ...
    # 在每个任务增量学习时执行的操作，包括更新总类别数、更新模型的输出层、设置部分梯度可训练性、
    # 记录模型参数信息、获取训练和测试数据集、创建 DataLoader 对象、调用 _train 方法进行训练、构建新任务的记忆数据。
    def incremental_train(self, data_manager):
        self._cur_task += 1

        # 计算总的类别数，包括已知的类别和将要增量学习的新类别
        ...
        # 从data_manager获取增量学习任务的训练数据和测试数据
        # 获取从已知类别到新类别的范围内的数据，模式为 "train"，同时追加已经存储的记忆数据
        ...
        # 如果系统有多个GPU，将模型并行化以充分利用多个GPU
        if len(self._multiple_gpus) > 1:
            self._network = nn.DataParallel(self._network, self._multiple_gpus)
        # 调用_train方法进行训练，传递训练数据和测试数据的DataLoader对象
    ...
    # 设置模型的状态，包括 TaskAgnosticExtractor 和 AdaptiveExtractors 的训练或评估状态。
